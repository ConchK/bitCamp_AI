...
from tensorflow.keras.layers import Conv1D   ### 시계열에서 많이 사용. 특성을 좁혀 나가야 하기 때문에 여러번 사용해야 함.
...
# model.add(Bidirectional(LSTM(100, return_sequences= True),  # > 원래 차원으로 되돌림.
                        # input_shape=(4,1)))      # 81600    >>2차원으로 아웃풋  다음 레이어에서 lstm 또 쓰려면 다시 3차원으로 바꿔야 함.
# model.add(LSTM(64))
# model.add(GRU(64))  #GRU도 사용 가능.

model.add(Conv1D(100, 2, input_shape= (4,1)))       # 300    
#한 줄로 펼쳐지기 때문에, kernel_size의 세로 size 필요없음. 그 외는 Conv2D랑 동일.
